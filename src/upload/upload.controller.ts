...
export class UploadController {
  ...
  async uploadPrescriptionFile(
    ...
    if (!customerId || !pharmacyId) throw new BadRequestException('customerId and pharmacyId are required');

    const url = await this.uploadService.uploadFile(file, `users/${customerId}/prescriptions`);
    const { requestId, chatId } = await this.uploadService.createPrescriptionChat({
      customerId, customerName, customerPhone, pharmacyId, pharmacyName, imageUrl: url
    });

    return { url, requestId, chatId };
  }

  @Post('prescription/base64')
  async uploadPrescriptionBase64(
    @Body('fileBase64') fileBase64: string,
    @Body('fileName') fileName: string,
    @Body('customerId') customerId: string,
    @Body('customerName') customerName: string,
    ...
    if (!fileBase64 || !fileName) throw new BadRequestException('fileBase64 and fileName are required');
    if (!customerId || !pharmacyId) throw new BadRequestException('customerId and pharmacyId are required');

    const buffer = Buffer.from(fileBase64, 'base64');
    const fauxFile = { buffer, originalname: fileName, mimetype: 'application/octet-stream' } as Express.Multer.File;

    const url = await this.uploadService.uploadFile(fauxFile, `users/${customerId}/prescriptions`);
    const { requestId, chatId } = await this.uploadService.createPrescriptionChat({
      customerId, customerName, customerPhone, pharmacyId, pharmacyName, imageUrl: url
    });
